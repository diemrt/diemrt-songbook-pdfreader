import React, { useEffect } from 'react';
import SearchBar from './SearchBar';

interface HeaderProps {
  filePath: string;
  onSearch: (query: string) => void;
  headerVisible: boolean;
  showHeader: () => void;
  hideHeader: () => void;
}

// Estrae il nome file dal path (es: "/foo/bar/06.18.pdf" => "06.18.pdf")
function extractFileName(filePath: string): string {
  const match = filePath.match(/(\d{2}\.\d{2})/);
  return match ? `Versione file: ${match[1]}` : '';
}

const Header: React.FC<HeaderProps> = ({ filePath, onSearch, headerVisible, showHeader, hideHeader }) => {
  const fileName = extractFileName(filePath);

  useEffect(() => {
    if (!headerVisible) return;
    const timeout = setTimeout(() => hideHeader(), 2000);
    return () => clearTimeout(timeout);
  }, [headerVisible, hideHeader]);

  return (
    <div
      className={`border-b border-gray-200 bg-white px-6 py-4 flex items-center gap-4 fixed top-0 left-0 w-full z-50 transition-opacity duration-200 ${headerVisible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      onMouseEnter={showHeader}
      onMouseLeave={hideHeader}
      onClick={showHeader}
      style={{ minHeight: 64 }}
    >
      <h1 className="text-2xl font-bold mr-2">Reader PDF Musicale</h1>
      <span className="ml-3 text-xs text-gray-400">{fileName}</span>
      <div className="flex-1 flex justify-end">
        <SearchBar onSearch={onSearch} />
      </div>
    </div>
  );
};

export default Header;
